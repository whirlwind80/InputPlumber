# InputPlumber Zotac Gaming Zone — 아카이브 (2026-08-25 ~ 2026-09-04)

이 파일은 `handoff.md`에서 분리한 **과거 기록**이다. 두 종류가 들어 있다:

1. **`hid-generic` 시절의 하드웨어 서술** — 2026-09-08에 벤더 드라이버 `zotac_zone_hid`가 들어오면서
   evdev 노드 구성, F16-F19 배치, 진짜 게임패드 노드가 전부 바뀌었다. 여기 적힌 하드웨어 관련
   서술은 **현재 기기와 다르다.**
2. **닫힌 PR 664/668의 리뷰 과정** — 두 PR 모두 2026-09-02/09-03에 머지 없이 CLOSED.

⚠️ **현재 사실을 확인할 때 이 파일을 근거로 쓰지 말 것.** 정본은:
- 코드베이스·이 기기에 대한 durable 사실 → `CLAUDE.md`
- 현재 작업 상태·다음 할 일 → `handoff.md`
- 리뷰 쟁점의 **최종 결론**(무엇이 채택됐고 무엇이 재현 안 됐는지) → `handoff.md`의 2026-09-09 세션
  §9-11 대조표. 이 파일은 그 결론에 이르기까지의 과정 기록이다.

여기 있는 내용 중 아직 유효한 것(벤더 설정 프로토콜, hidraw 레벨 사실, 디버깅 기법)은 이미
`CLAUDE.md`로 옮겨져 있다.
...
세션"을 이날 진행했고, 컨트롤러를 정상 동작 상태로 되돌렸다.** 상세는 **`handoff.md`의
"★ 2026-09-08 세션"** 참고(이 아카이브에는 없다). 하드웨어 토폴로지·F16-F19 배치·진짜 게임패드 노드가 전부 바뀌었으므로, **이 문서와
